...
## Runtime observations (Chrome, `--enable-features=WebMCP`)

| Observation | What we saw | Where |
|---|---|---|
| Long-running `execute` | A tool that awaits a promise resolved by a user click returns normally after `[ ]` s of waiting. We cap at 90 s ourselves. | `packages/quest-tools/src/controller.ts` run(), `tests/e2e.mjs` 7a/7b |
...
| `execute(input, { signal })` | The second argument is `[present / absent]` in Chrome `[version]`. We accept it and forward it to the confirm dialog so an agent cancel closes the dialog. | `controller.ts`, `ui/confirm-dialog.ts` |
...
| Budgets | Output over 1,500 characters is truncated by us before it leaves the page; the runtime `[does / does not]` enforce it. | `packages/quest-tools/src/envelope.ts` |
...
>
> **Requirements we would put on `requestUserInteraction()` or similar, from running this in Chrome [version]:**
